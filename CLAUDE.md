...
To get PR review comments with their resolved status, use GraphQL directly (not REST API):
...
**Why GraphQL**: The REST endpoints (`gh api repos/.../pulls/N/comments`) don't include resolved status. Only the GraphQL `reviewThreads` query provides both comments and their resolution state.

**Quick reference**:
- `gh pr view --comments` - Only PR-level comments, not inline code review comments
- `gh api repos/.../pulls/N/comments` - Inline comments but NO resolved status
- `gh api graphql` with `reviewThreads` - Full data: comments, paths, lines, AND resolved status

### Handling Review Comments

When user signals review ("I made a review", "Check PR comments"):

1. Read PR comments using GraphQL query above (filter for `isResolved: false`)
2. For each comment:
