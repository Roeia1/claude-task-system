...
title: Update plugin skills and publish v3.0.0
status: ready
...
tasks:
  ...
  - id: t5
    ...
    status: pending
  - id: t6
    title: Publish @saga-ai/dashboard@3.0.0
    status: pending
  - id: t7
    title: Publish plugin v3.0.0
    status: pending
---

## Context

This story finalizes the CLI dashboard refactor epic by updating all plugin skills and agents to use the new plugin-scripts infrastructure instead of `npx @saga-ai/cli` commands, then publishing the new packages.
...
Additionally, this story handles the npm package transition: deprecating/deleting `@saga-ai/cli` and publishing the new `@saga-ai/dashboard` package at version 3.0.0, along with updating the plugin version.
...
- Deleting `@saga-ai/cli` from npm registry (or deprecating if deletion not possible)
- Publishing `@saga-ai/dashboard@3.0.0` to npm
- Publishing plugin v3.0.0
...
- `@saga-ai/dashboard@3.0.0` published to npm
- Plugin v3.0.0 tagged and ready
...
- [ ] `@saga-ai/dashboard@3.0.0` is published and installable via `npm install @saga-ai/dashboard`
...
### t5: Delete @saga-ai/cli from npm
...
### t6: Publish @saga-ai/dashboard@3.0.0

**Guidance:**
- Ensure the package builds successfully before publishing
- Run `pnpm build` in packages/dashboard
- Run `pnpm publish` or `npm publish` with appropriate access
- Verify the package is accessible after publish

**References:**
- `packages/dashboard/package.json` - Package configuration
- npm publish documentation

**Avoid:**
- Publishing without building first
- Publishing with private: true in package.json

**Done when:**
- `npm view @saga-ai/dashboard` shows version 3.0.0
- Package can be installed with `npm install @saga-ai/dashboard`

### t7: Publish plugin v3.0.0

**Guidance:**
- Create a git tag for v3.0.0
- Update any plugin distribution mechanism (if applicable)
- Ensure plugin.json reflects 3.0.0

**References:**
- `plugin/plugin.json` - Plugin manifest
- Existing release workflow in `.claude/skills/publish/`

**Avoid:**
- Tagging before all changes are committed
- Missing any files in the commit

**Done when:**
- Git tag v3.0.0 exists
- Plugin is ready for distribution at v3.0.0
