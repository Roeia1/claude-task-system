...
describe.skipIf(!hasTmux)("sessions integration", () => {
	...
	describe("createSession", () => {
		it("should create a tmux session with correct name pattern", async () => {
			...
			expect(result.outputFile).toBe(
				join(OUTPUT_DIR, `${result.sessionName}.out`),
			);
		...
		it("should reject invalid epic slug", () => {
			expect(() =>
				createSession("Invalid_Epic", testStory, "echo test"),
			).toThrow(INVALID_EPIC_SLUG_PATTERN);
		});

		it("should reject invalid story slug", () => {
			expect(() =>
				createSession(testEpic, "Invalid_Story", "echo test"),
		...
		it("should reject empty slug", () => {
			expect(() => createSession("", testStory, "echo test")).toThrow(
				INVALID_EPIC_SLUG_PATTERN,
			);
		});
	});
	...
	describe("listSessions", () => {
		it("should return empty array when no saga sessions exist", async () => {
			...
			const testSessions = sessions.filter(
				(s) =>
					s.name.startsWith("saga__test-epic__") ||
					s.name.startsWith("saga__integration__"),
			);
		...
		it("should only return sessions with saga__ prefix", async () => {
			...
			spawnSync(
				"tmux",
				["new-session", "-d", "-s", "other-session", "sleep 5"],
				{
					encoding: "utf-8",
				},
			);
			...
			try {
				...
				const nonSagaSessions = sessions.filter(
					(s) => !s.name.startsWith("saga__"),
				);
	...
	describe("getSessionStatus", () => {
		...
		it("should return running: false for non-existent session", async () => {
			const status = await getSessionStatus(
				"saga__non-existent__session__99999",
			);
