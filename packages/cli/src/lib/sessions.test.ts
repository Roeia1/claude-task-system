...
describe("sessions", () => {
	...
	describe("shellEscapeArgs", () => {
		it("should escape and join multiple arguments", () => {
			expect(shellEscapeArgs(["echo", "hello", "world"])).toBe(
				"'echo' 'hello' 'world'",
			);
		});
		...
		it("should handle arguments with spaces", () => {
			expect(
				shellEscapeArgs(["command", "--path", "/some path/with spaces"]),
		...
		it("should handle mixed safe and unsafe arguments", () => {
			expect(
				...
				"'saga' 'implement' 'my-story' '--path' '/Users/test'\\''s folder'",
			);
		});
	...
	describe("createSession", () => {
		it("should reject invalid epic slug", () => {
			expect(() =>
				createSession("Invalid_Epic", "valid-story", "echo hello"),
			).toThrow(INVALID_EPIC_SLUG_PATTERN);
		});

		it("should reject invalid story slug", () => {
			expect(() =>
				createSession("valid-epic", "Invalid_Story", "echo hello"),
			).toThrow(INVALID_STORY_SLUG_PATTERN);
		});
		...
		it("should return session name and output file path on success", () => {
			...
			expect(result.outputFile).toBe(
				`/tmp/saga-sessions/${result.sessionName}.out`,
			);
		});
	...
	describe("listSessions", () => {
		...
		it("should include output file path for each session", () => {
			...
			expect(result[0].outputFile).toBe(
				"/tmp/saga-sessions/saga__epic1__story1__1234.out",
			);
		});
	});
	...
	describe("streamLogs", () => {
		...
		it("should throw error when output file does not exist", () => {
			...
			expect(() => streamLogs("saga__epic1__story1__1234")).toThrow(
				OUTPUT_FILE_NOT_FOUND_PATTERN,
			);
		});
	...
	describe("parseSessionName", () => {
		...
		it("should handle slugs containing hyphens", () => {
			const result = parseSessionName(
				"saga__epic-with-hyphens__story-with-hyphens__99999",
			);
			expect(result).toEqual({
	...
	describe("buildSessionInfo", () => {
		...
		it("should return DetailedSessionInfo for valid running session with output file", async () => {
			...
			const result = assertDefined(
				await buildSessionInfo(sessionName, "running"),
			);
		...
		it("should include endTime for completed sessions", async () => {
			...
			const result = assertDefined(
				await buildSessionInfo(sessionName, "completed"),
			);
		...
		it("should truncate outputPreview to max 500 chars", async () => {
			...
			expect(assertDefined(result.outputPreview).length).toBeLessThanOrEqual(
				MAX_PREVIEW_LENGTH,
			);
		});
