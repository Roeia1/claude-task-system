...
`,
			...
			try {
				const unsubscribedMsg = await waitForMessage(
					unsubscribedWs,
					SHORT_MESSAGE_TIMEOUT_MS,
				);
...
`,
			...
			expect(Array.isArray((msg.data as { tasks: unknown[] }).tasks)).toBe(
				true,
			);
	...
	describe("subscribe:logs and unsubscribe:logs", () => {
		...
		it(
			...
			async () => {
				...
				expect((initialMsg.data as { isInitial: boolean }).isInitial).toBe(
					true,
				);
