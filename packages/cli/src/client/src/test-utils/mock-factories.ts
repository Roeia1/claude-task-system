...
function createMockSession(
	...
	const startTime =
		overrides.startTime ?? new Date(Date.now() - startOffset).toISOString();
	...
	const outputAvailable =
		overrides.outputAvailable ?? preset !== "output-unavailable";
	const outputPreview = getOutputPreviewForPreset(
		preset,
		overrides.outputPreview,
	);

	return {
		name:
			overrides.name ??
			`saga__test-epic__story-${id}__${BASE_SESSION_PID + id}`,
		epicSlug: overrides.epicSlug ?? "test-epic",
 ...
 */
function getOutputPreviewForPreset(
