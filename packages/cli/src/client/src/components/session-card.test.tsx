...
describe("SessionCard", () => {
	...
	describe("data display", () => {
		...
		it("displays output preview", () => {
			...
			expect(
				screen.getByText(INSTALLING_DEPENDENCIES_PATTERN),
			).toBeInTheDocument();
			expect(
				screen.getByText(COMPILING_TYPESCRIPT_PATTERN),
			).toBeInTheDocument();
		});
	...
	describe("output preview truncation", () => {
		...
		it("truncates output to max 500 characters", () => {
			...
			expect(preElement.textContent?.length).toBeLessThanOrEqual(
				MAX_OUTPUT_LENGTH,
			);
		});
	});
	...
	describe("navigation", () => {
		it("renders as a link to story detail with sessions tab", () => {
			...
			expect(link).toHaveAttribute(
				"href",
				"/epic/test-epic/story/test-story?tab=sessions",
			);
		});
