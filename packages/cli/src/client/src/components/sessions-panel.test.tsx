...
describe("SessionsPanel", () => {
	...
	describe("loading state", () => {
		it("shows skeleton while fetching sessions", () => {
			...
			renderWithProviders(
				<SessionsPanel epicSlug="test-epic" storySlug="test-story" />,
			);
	...
	describe("empty state", () => {
		it("shows empty message when no sessions exist", async () => {
			...
			await waitFor(() => {
				expect(
					screen.queryByTestId("sessions-panel-skeleton"),
				).not.toBeInTheDocument();
			});

			expect(screen.getByTestId("sessions-panel-empty")).toBeInTheDocument();
			expect(
				screen.getByText("No sessions found for this story"),
			).toBeInTheDocument();
		});
	});
	...
	describe("API call", () => {
		it("fetches sessions with epicSlug and storySlug query params", async () => {
			...
			renderWithProviders(
				<SessionsPanel epicSlug="my-epic" storySlug="my-story" />,
			);
	...
	describe("error handling", () => {
		it("shows error state when API returns error", async () => {
			...
			expect(
				screen.getByRole("button", { name: RETRY_BUTTON_PATTERN }),
			).toBeInTheDocument();
		});
