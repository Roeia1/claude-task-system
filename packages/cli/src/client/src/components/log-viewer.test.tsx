...
describe("LogViewer", () => {
	...
	describe("outputAvailable prop", () => {
		...
		it("does not show unavailable message when outputAvailable is true", () => {
			...
			expect(
				screen.queryByText(OUTPUT_UNAVAILABLE_REGEX),
	...
	describe("virtual scrolling", () => {
		it("uses virtualization for large content", () => {
			...
			const lines = Array.from(
				{ length: LARGE_CONTENT_LINE_COUNT },
				(_, i) => `Log line ${i + 1}`,
			);
...
Line 3`;
	...
	describe("status indicator", () => {
		it("shows streaming indicator with animation for running sessions", () => {
			...
			const streamingIndicator = screen.getByTestId(
		...
		it("does not show status indicator when output is unavailable", () => {
			...
			expect(
				screen.queryByTestId("status-indicator-streaming"),
			).not.toBeInTheDocument();
			expect(
				screen.queryByTestId("status-indicator-complete"),
			).not.toBeInTheDocument();
		});
	});
	...
	describe("edge cases", () => {
		...
		it("does not show loading when initialContent is provided", () => {
			...
			expect(
				screen.queryByTestId("log-viewer-skeleton"),
			).not.toBeInTheDocument();
		});
