	...
	return (
		<div className="flex items-center justify-between px-4 py-2 border-b border-bg-light">
			...
			<button
				...
				title={
					autoScroll ? "Autoscroll locked to bottom" : "Autoscroll unlocked"
				}
			...
			>
				{autoScroll ? (
					...
					<Unlock
						...
						data-testid="autoscroll-icon-disabled"
					/>
				)}
...
function LogViewerError({ error }: { error: string }) {
	return (
		...
		>
			...
			<span className="text-text-muted text-xs max-w-xs text-center">
				{error}
			</span>
		</div>
	);
...
export function LogViewer({
	...
	return (
		<div
			data-testid="log-viewer"
			className="flex flex-col h-full bg-bg-dark rounded-md"
		>
			<LogViewerHeader
