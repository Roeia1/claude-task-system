...
const Showcase: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByText("Edge Cases (Long Slugs)"),
		).toBeInTheDocument();
		...
		const dashboardRestructureTexts = canvas.getAllByText(
		...
		const longEpicSlugs = canvas.getAllByText(
			"implement-authentication-and-authorization-system",
		);
		await expect(longEpicSlugs.length).toBe(2); // Appears in both long slug examples
		await expect(
			canvas.getByText("add-oauth-provider-integration"),
		).toBeInTheDocument();
...
const Playground: Story = {
			...
			<div className="space-y-6">
				<section>
					<h3 className="text-sm font-medium text-text-muted mb-2">
						{args.customRoute
							? "Custom Route"
							: routeConfigs[args.preset].description}
					</h3>
	...
	play: async ({ canvasElement, args }) => {
		...
		if (args.preset === "root" && !args.customRoute) {
			await expect(
				canvas.getByTestId("breadcrumb-home-icon"),
			).toBeInTheDocument();
		...
		if (args.preset === "epicDetail" && !args.customRoute) {
			...
			await expect(
				canvas.getByText("dashboard-restructure"),
			).toBeInTheDocument();
