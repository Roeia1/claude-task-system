...
export function EpicContent({ content }: EpicContentProps) {
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen} data-testid="epic-content">
      <div class="border border-border-muted rounded-lg bg-bg-light/30">
        <CollapsibleTrigger asChild={true}>
          <button
            type="button"
            class="w-full flex items-center justify-between p-4 hover:bg-bg-light/50 transition-colors rounded-t-lg"
          >
            <h2 class="text-lg font-semibold text-text">Epic Documentation</h2>
            {isOpen ? (
