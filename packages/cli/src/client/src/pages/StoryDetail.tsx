...
function ContentSkeleton() {
	return (
		<div
			className="animate-pulse space-y-4"
			data-testid="story-content-skeleton"
		>
...
function TaskItem({ task }: { task: StoryDetailType["tasks"][0] }) {
	...
	return (
		<div className="flex items-center gap-3 py-2 px-3 rounded-md hover:bg-bg-light/50">
			...
			<Badge className={`ml-auto text-xs ${badgeClass}`}>
...
function JournalSection({
	...
	return (
		<div className="space-y-3">
			<h3
				className={`text-sm font-semibold ${colorClass} flex items-center gap-2`}
			>
...
function StoryDetail() {
	...
	const { currentStory, loading, notFound, error } = useStoryFetch(
		epicSlug,
		storySlug,
	);
