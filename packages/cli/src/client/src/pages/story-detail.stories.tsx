...
function StoryHeader({
	...
	const completedCount = story.tasks.filter(
			...
			<div className="space-y-6">
				...
				<Tabs defaultValue="journal" className="w-full">
					...
					<TabsContent value="journal">
						<JournalTabContent
							journal={storyWithBlocker.journal}
							hasBlocker={true}
						/>
					</TabsContent>
				</Tabs>
...
export const Showcase: StoryDetailStory = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByTestId("story-header-skeleton"),
		).toBeInTheDocument();
		await expect(
			canvas.getByTestId("story-content-skeleton"),
		).toBeInTheDocument();
		...
		// Verify empty state
		await expect(
			canvas.getByText("No tasks defined for this story."),
		).toBeInTheDocument();
		...
		await expect(
			canvas.getByText("Blocked: Missing API credentials"),
		).toBeInTheDocument();

		// Verify completed state
		await expect(
			canvas.getByText("Setup Project Structure"),
		).toBeInTheDocument();
...
export const Playground: StoryDetailStory = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByRole("tab", { name: "Tasks" }),
		).toBeInTheDocument();
		await expect(
			canvas.getByRole("tab", { name: "Story Content" }),
		).toBeInTheDocument();
		await expect(
			canvas.getByRole("tab", { name: "Journal" }),
		).toBeInTheDocument();
		await expect(
			canvas.getByRole("tab", { name: "Sessions" }),
		).toBeInTheDocument();
		...
		const epicLinks = canvas
			.getAllByRole("link")
			.filter(
				(link) => link.getAttribute("href") === "/epic/dashboard-restructure",
			);
...
export const Loading: StoryDetailStory = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(placeholders.length).toBeGreaterThanOrEqual(
			MIN_SKELETON_PLACEHOLDER_COUNT,
		);
...
export const NotFound: StoryDetailStory = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(backLinks[0]).toHaveAttribute(
			"href",
...
export const Populated: StoryDetailStory = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByText("Install and configure Storybook 10.x"),
		).toBeInTheDocument();
		await expect(
			canvas.getByText("Create stories for StoryDetail page"),
		).toBeInTheDocument();
...
export const WithBlocker: StoryDetailStory = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByText("Session: 2026-01-28 09:00 UTC"),
		).toBeInTheDocument();
...
export const Completed: StoryDetailStory = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByText("Setup linting and formatting"),
		).toBeInTheDocument();
...
export const WithContent: StoryDetailStory = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByText("Story Content", { selector: ".text-lg" }),
		).toBeInTheDocument();

		// Verify markdown content is rendered
		await expect(
			canvas.getByRole("heading", { name: "Context" }),
		).toBeInTheDocument();
		await expect(
			canvas.getByRole("heading", { name: "Acceptance Criteria" }),
		).toBeInTheDocument();
...
export const EmptyJournal: StoryDetailStory = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByText("No journal entries yet."),
		).toBeInTheDocument();
	},
};
