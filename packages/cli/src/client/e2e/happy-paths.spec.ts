...
test.describe("Epic List", () => {
	test("displays all fixture epics with correct information", async ({
		page,
	}) => {
		await page.goto("/");
...
test.describe("Epic Detail", () => {
	test("displays epic with stories via direct navigation", async ({ page }) => {
		...
		await expect(
			page.getByText("User Authentication Implementation"),
		).toBeVisible();
	...
	test("displays story status badges correctly", async ({ page }) => {
		...
		const apiStoryCard = page.locator(
			'a[href="/epic/feature-development/story/api-design"]',
		);
	...
	test("empty epic shows no stories message", async ({ page }) => {
		...
		await expect(
			page.locator('h1.text-2xl:has-text("Empty Epic")'),
		).toBeVisible();
...
test.describe("Story Detail", () => {
	test("displays story via direct navigation", async ({ page }) => {
		...
		// Verify story title
		await expect(
			page.locator('h1:has-text("User Authentication Implementation")'),
		).toBeVisible();
		...
		await expect(
			page.getByRole("tab", { name: JOURNAL_TAB_PATTERN }),
		).toBeVisible();
	});
	...
	test("navigates back to epic from breadcrumb", async ({ page }) => {
		...
		await expect(
			page.locator('a[href="/epic/feature-development"]').first(),
		).toBeVisible();
...
test.describe("WebSocket Real-time Updates", () => {
	...
	test("epic list updates when story status changes", async ({
		page,
		fixtureUtils,
	}) => {
		await page.goto("/");
	...
	test("story detail updates when story file changes", async ({
		page,
		fixtureUtils,
	}) => {
		await page.goto("/epic/feature-development/story/auth-implementation");
...
test.describe("Sessions Tab", () => {
	test("displays Sessions tab in story detail page", async ({ page }) => {
		...
		await expect(
			page.getByRole("tab", { name: SESSIONS_TAB_PATTERN }),
		).toBeVisible();
	});
	...
	test("switches to Sessions tab and shows content", async ({ page }) => {
		...
		await page.route("**/api/sessions*", async (route) => {
			...
			const epicSlug =
				url.searchParams.get("epicSlug") || "feature-development";
			const storySlug =
				url.searchParams.get("storySlug") || "auth-implementation";
	...
	test("navigates to Sessions tab via URL query parameter", async ({
		page,
	}) => {
		// Mock sessions API
		await page.route("**/api/sessions*", async (route) => {
...
test.describe("ActiveSessions on Home Page", () => {
	test("shows ActiveSessions section when running sessions exist", async ({
		page,
	}) => {
	...
	test("clicking session card navigates to story with sessions tab", async ({
		page,
	}) => {
		// Mock sessions API with running session
		await page.route("**/api/sessions*", async (route) => {
