...
test.describe("Error States", () => {
	test.describe("Epic List Page", () => {
		...
		test("should show toast notification on network failure", async ({
			page,
		}) => {
		...
		test("should display empty state after error when no cached data", async ({
			page,
		}) => {
			// Mock the API to return a 500 error
			await mockApiError(
	...
	test.describe("Epic Detail Page", () => {
		test("should show 404 error page for non-existent epic", async ({
			page,
		}) => {
			// Mock 404 for non-existent epic
			await mockApiError(
				page,
				"**/api/epics/non-existent-epic",
				HTTP_NOT_FOUND,
				"Epic not found",
			);
			...
			await expect(
				page.getByRole("heading", { name: "Epic not found" }),
			).toBeVisible();
			await expect(
				page.getByText('The epic "non-existent-epic" does not exist.'),
			).toBeVisible();

			// Should have a link back to epic list
			await expect(
				page.getByRole("link", { name: BACK_TO_EPIC_LIST_REGEX }),
			).toBeVisible();
		});
		...
		test("should allow navigation back to epic list from error page", async ({
			page,
		}) => {
	...
	test.describe("Story Detail Page", () => {
		test("should show 404 error page for non-existent story", async ({
			page,
		}) => {
			...
			await expect(
				page.getByText(
					'The story "non-existent-story" does not exist in epic "test-epic".',
				),
			).toBeVisible();

			// Should have a link back to epic
			await expect(
				page.getByRole("link", { name: BACK_TO_EPIC_REGEX }),
			).toBeVisible();
		});
		...
		test("should show toast notification on network failure", async ({
			...
			await mockNetworkFailure(
				page,
				"**/api/stories/test-epic/network-error-story",
			);
		...
		test("should allow navigation back to epic from error page", async ({
			page,
		}) => {
			// Mock the epic detail
			await mockEpicDetail(
				page,
				createMockEpic({ slug: "test-epic", title: "Test Epic" }),
			);
	...
	test.describe("Mixed Error Scenarios", () => {
		test("should handle error on one page and success on another", async ({
			page,
		}) => {
			// Epic list works
			await mockEpicList(page, [
		...
		test("should recover after navigating away from error page", async ({
			page,
		}) => {
			...
			await expect(
				page.getByRole("heading", { name: "Working Epic Detail" }),
			).toBeVisible();
		});
	});
