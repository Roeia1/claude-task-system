...
test.describe("ActiveSessions on Home Page", () => {
	test("should not show ActiveSessions section when no running sessions", async ({
		page,
	}) => {
		const epics = [
	...
	test("should display session cards with story and epic slugs", async ({
		page,
	}) => {
		const epics = [
			createMockEpicSummary({ slug: "epic-one", title: "Epic One" }),
	...
	test("should navigate to story detail with ?tab=sessions when clicking session card", async ({
		...
		await expect(page).toHaveURL(
			"/epic/epic-one/story/story-alpha?tab=sessions",
		);
	});

	test("should only show running sessions, not completed ones", async ({
		page,
	}) => {
		const epics = [
			createMockEpicSummary({ slug: "epic-one", title: "Epic One" }),
		...
		await expect(
			activeSessionsSection.getByText("story-gamma"),
		).not.toBeVisible();
	});
