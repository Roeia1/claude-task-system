...
describe('find command', () => {
  ...
  describe('epic search (--type epic)', () => {
    it('should find epic by exact slug', () => {
      ...
      const result = runCli(['find', 'user-authentication', '--type', 'epic', '--path', testDir]);
    ...
    it('should find epic by partial match', () => {
      ...
      const result = runCli(['find', 'auth', '--type', 'epic', '--path', testDir]);
    ...
    it('should exit 1 when no epic found', () => {
      ...
      const result = runCli(['find', 'nonexistent', '--type', 'epic', '--path', testDir]);
