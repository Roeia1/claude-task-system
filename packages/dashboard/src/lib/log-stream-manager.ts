...
export type SendToClientFn = (
	ws: WebSocket,
	message: LogsDataMessage | LogsErrorMessage,
) => void;
...
export class LogStreamManager {
	 ...
	 * @param outputFile - Path to the session output file
	 */
	private async sendIncrementalContent(
		sessionName: string,
		outputFile: string,
	): Promise<void> {
		...
		const newContent = await this.readFromPosition(
			outputFile,
			lastPosition,
			currentSize,
		);
	...
	async notifySessionCompleted(sessionName: string): Promise<void> {
		...
		try {
			if (existsSync(outputFile)) {
				...
				if (currentSize > lastPosition) {
					finalContent = await this.readFromPosition(
						outputFile,
						lastPosition,
						currentSize,
					);
