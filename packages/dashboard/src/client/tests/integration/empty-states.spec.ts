...
test.describe("Empty States", () => {
	test.describe("Epic List Page", () => {
		test("should show empty state message when no epics exist", async ({
			page,
		}) => {
		...
		test("should show archived epics when toggle is enabled", async ({
			page,
		}) => {
			// Mock epics where all are archived
			const archivedEpics = [
	...
	test.describe("Epic Detail Page", () => {
		test("should show empty state when epic has no stories", async ({
			page,
		}) => {
		...
		test("should show epic header even when no stories exist", async ({
			page,
		}) => {
			// Create an epic with no stories
			const emptyEpic = createMockEpic({
			...
			await expect(
				page.getByRole("heading", { name: "Epic With No Stories" }),
			).toBeVisible();
	...
	test.describe("Story Detail Page", () => {
		test("should show empty state when story has no tasks", async ({
			page,
		}) => {
			...
			await expect(
				page.getByText("No tasks defined for this story."),
			).toBeVisible();
		});

		test("should show empty state when story has no content", async ({
			page,
		}) => {
		...
		test("should display story header correctly even with all empty states", async ({
			...
			await expect(
				breadcrumb.getByRole("link", { name: "test-epic" }),
			).toBeVisible();
		});
	});
	...
	test.describe("Multiple Empty States Combined", () => {
		...
		test("should navigate from empty story list to generate stories guidance", async ({
			...
			// Verify guidance text is actionable
			const generateStoriesCode = page.locator(
				'code:has-text("/generate-stories")',
			);
		...
		test("should handle transition from data to empty state on navigation", async ({
			...
			await expect(
				page.getByText("No stories in this epic."),
			).not.toBeVisible();
