...
test.describe("Epic Content Display", () => {
	test.describe("Content visibility", () => {
		test("should display epic content section when epic has content", async ({
			page,
		}) => {
			...
			await expect(
				page.getByRole("button", { name: "Epic Documentation" }),
			).toBeVisible();

			// Verify markdown content is rendered (inside the prose container)
			const markdownContent = epicContent.locator(".prose");
			await expect(markdownContent.locator("h1")).toContainText(
				"Epic Overview",
			);
			await expect(
				page.getByText("This is the main description."),
			).toBeVisible();
		});

		test("should not display epic content section when epic has no content", async ({
			page,
		}) => {
			...
			await expect(
				page.getByRole("heading", { name: "Epic Without Content" }),
			).toBeVisible();
		...
		test("should not display epic content section when content is empty string", async ({
			...
			await expect(
				page.getByRole("heading", { name: "Epic With Empty Content" }),
			).toBeVisible();
	...
	test.describe("Collapsible behavior", () => {
		test("should show content expanded by default", async ({ page }) => {
			...
			await expect(
				page.getByText("This content should be visible by default."),
			).toBeVisible();
		});

		test("should collapse content when toggle button is clicked", async ({
			page,
		}) => {
			const epic = createMockEpic({
			...
			await page
				.getByRole("button", { name: EPIC_DOCUMENTATION_REGEX })
				.click();
		...
		test("should expand content when toggle button is clicked again", async ({
			page,
		}) => {
			const epic = createMockEpic({
				slug: "toggle-epic",
	...
	test.describe("Content positioning", () => {
		test("should display epic content between header and stories", async ({
			page,
		}) => {
			const epic = createMockEpic({
				slug: "layout-epic",
			...
			await expect(
				page.getByRole("heading", { name: "Layout Epic" }),
			).toBeVisible();
			...
			const epicContent = page.getByTestId("epic-content");
			await expect(
				epicContent.getByRole("button", { name: "Epic Documentation" }),
			).toBeVisible();
			await expect(
				epicContent.getByText("Layout description here."),
			).toBeVisible();
		});
	});
	...
	test.describe("Markdown rendering", () => {
		...
		test("should render code blocks in markdown content", async ({ page }) => {
			...
			await expect(
				epicContent.getByText('const hello = "world";'),
			).toBeVisible();
		});
