...
const Showcase: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		const dashboardTitles = canvas.getAllByText(
		...
		await expect(
			canvas.getByText("Old Feature (Archived)"),
		).toBeInTheDocument();
...
const Playground: Story = {
	argTypes: {
		...
		showArchived: {
			control: "boolean",
			description:
				"Whether to show archived epics (when state is with-archived)",
		},
	},
	args: {
	...
	render: (args) => {
		...
		const epics =
			state === "with-archived" && showArchived
				? epicsWithArchived
				: sampleEpics;
