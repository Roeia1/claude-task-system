...
export const Showcase: EpicDetailStory = {
				...
				<div className="space-y-6">
					...
					<div className="text-center py-12">
						...
						<p className="text-text-muted">
							Run <code className="text-primary">/generate-stories</code> to
							create stories.
						</p>
					</div>
				</div>
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByText(NON_EXISTENT_EPIC_PATTERN),
		).toBeInTheDocument();

		// Verify empty state
		await expect(
			canvas.getByText("No stories in this epic."),
		).toBeInTheDocument();
		...
		await expect(
			canvas.getByText("Visual Regression Testing"),
		).toBeInTheDocument();
...
export const Populated: EpicDetailStory = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByText("Playwright Integration Tests"),
		).toBeInTheDocument();
		await expect(
...
export const AllCompleted: EpicDetailStory = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByText("Authentication Migration"),
		).toBeInTheDocument();
...
export const WithBlockers: EpicDetailStory = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByText("Authentication Endpoint"),
		).toBeInTheDocument();
