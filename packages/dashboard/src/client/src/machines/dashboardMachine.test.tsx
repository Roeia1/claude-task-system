...
const sampleStory: StoryDetail = {
	...
	journal: [
		{ type: "session", title: "Started work", content: "Initial session" },
	],
...
describe("dashboardMachine", () => {
	describe("graph analysis", () => {
		...
		it("should have primary states reachable via graph traversal", () => {
			const paths = getShortestPaths(graphMachine, {
				events: transitionEvents,
			});
			const reachableStates = new Set(
				paths.map((p) => serializeStateValue(p.state.value)),
			);
		...
		it("should find shortest paths to each state", () => {
			...
			for (const path of paths) {
				...
				const userEventSteps = path.steps.filter(
					(s) => s.event.type !== "xstate.init",
				);
		...
		it("should have multiple valid paths through the machine", () => {
			const paths = getSimplePaths(graphMachine, {
				...
				toState: (state) =>
					serializeStateValue(state.value) === "active.connected",
			});
	...
	describe("path-based state transitions", () => {
		...
		it.each(
			paths.map((path) => ({
				...
				steps: path.steps,
				pathDescription:
					path.steps.map((s) => s.event.type).join(" -> ") || "(initial)",
			})),
		)('should reach "$targetState" via: $pathDescription', ({
			targetState,
			steps,
		}) => {
	...
	describe("state invariants", () => {
		...
		it.each(
			...
			for (const step of steps) {
				actor.send(step.event);
				expect(actor.getSnapshot().context.retryCount).toBeGreaterThanOrEqual(
					0,
				);
			}
	...
	describe("async transitions", () => {
		...
		it("loading -> reconnecting on WebSocket error (triggers retry)", async () => {
			...
			actor.send({ type: "CONNECT" });
			await waitFor(actor, (s) => s.value === "reconnecting", {
				timeout: 1000,
			});
		...
		it("reconnecting -> loading after backoff delay (when canRetry)", async () => {
			...
			await waitFor(actor, (s) => s.matches({ active: "loading" }), {
				timeout: 100,
			});
	...
	describe("data events", () => {
		...
		describe("in connected state", () => {
			...
			it("STORY_UPDATED updates currentStory when slug matches", async () => {
				...
				expect(actor.getSnapshot().context.currentStory?.title).toBe(
					"Updated Story",
				);
			...
			it("SESSIONS_UPDATED updates sessions", async () => {
				...
				const updatedSessions = [
					{ ...sampleSessions[0], status: "completed" as const },
				];
	...
	describe("subscription management", () => {
		...
		it("does not restore unsubscribed stories after reconnect", async () => {
			...
			const story1Messages = (
				subscriptionCapture.messages as SubscriptionMessage[]
			).filter(
				(m) => m.type === "subscribe:story" && m.storySlug === "story-1",
			);
			const story2Messages = (
				subscriptionCapture.messages as SubscriptionMessage[]
			).filter(
