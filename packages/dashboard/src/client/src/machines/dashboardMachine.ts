...
>(({ sendBack, input, receive }) => {
	...
	const connect = () => {
		try {
			...
			ws.onmessage = (event) =>
				handleWebSocketMessage(event, lastPongRef, sendBack);
		} catch {
		...
		addSubscription: assign({
			subscribedStories: (
				...
				const exists = context.subscribedStories.some(
					(s) =>
						s.epicSlug === params.epicSlug && s.storySlug === params.storySlug,
				);
				if (exists) {
