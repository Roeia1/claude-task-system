...
const Showcase: Story = {
					...
					<div className="divide-y divide-border-muted rounded-lg border border-border-muted">
						...
						<div className="p-2">
							...
							<TaskItem
								task={createMockTask("pending", {
									title:
										'Handle <script> tags & "quoted" strings in task names',
								})}
							/>
						</div>
					</div>
					...
					<div className="flex flex-col gap-3">
						...
						<div className="flex items-center gap-3">
							...
							<span className="text-primary">
								In Progress - primary filled circle
							</span>
						</div>
						<div className="flex items-center gap-3">
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByText("Status Icons Reference"),
		).toBeInTheDocument();

		// Verify task titles
		await expect(
			canvas.getByText("Write unit tests for API endpoints"),
		).toBeInTheDocument();
		await expect(
			canvas.getByText("Implement authentication flow"),
		).toBeInTheDocument();
		await expect(
...
const Playground: Story = {
	...
	render: (_, { args }) => {
		...
		const preset =
			(args as unknown as { preset?: TaskPreset }).preset ?? "pending";
		...
		return (
			<div className="space-y-4">
				<div className="text-sm text-text-muted">
					Preset:{" "}
					<span className="font-medium text-text">{presetToLabel(preset)}</span>
				</div>
