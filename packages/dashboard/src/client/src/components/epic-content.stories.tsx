...
function GfmFeaturesSection(): React.JSX.Element {
	return (
		<div className="space-y-2">
			...
			<p className="text-xs text-muted-foreground">
				Strikethrough text and link rendering
			</p>
...
function CollapsibleBehaviorSection(): React.JSX.Element {
	return (
		<div className="space-y-2">
			...
			<p className="text-xs text-muted-foreground">
				Click the header to expand/collapse content
			</p>
			<EpicContent
...
export const Showcase: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(epicContents.length).toBeGreaterThan(
			SHOWCASE_MIN_EPIC_CONTENT_COUNT,
		);
		...
		const strikethroughTexts = canvas.getAllByText(
			"This feature has been deprecated.",
		);
...
export const Playground: Story = {
	...
	render: ({ preset, customContent }) => {
		...
		const displayLabel = customContent
			? "Custom Content"
			: presetToLabel(preset);
	...
	play: async ({ canvasElement, args }) => {
		...
		const displayLabel = args.customContent
			? "Custom Content"
			: presetToLabel(args.preset);
		await expect(
			canvas.getByText(displayLabel, { exact: false }),
		).toBeInTheDocument();
