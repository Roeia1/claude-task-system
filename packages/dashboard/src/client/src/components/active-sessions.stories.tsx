	...
	return (
		<div className="space-y-2">
			...
			<p className="text-sm text-text-muted">
				Skeleton loading component displayed while session data is being
				fetched.
			</p>
...
function EmptyStateSection() {
	return (
		<div className="space-y-2">
			...
			<p className="text-sm text-text-muted">
				ActiveSessions returns null when no running sessions exist. The section
...
function SessionStatesSection() {
	...
	return (
		<div className="space-y-2">
			...
			<p className="text-sm text-text-muted">
				All 5 session preset states displayed side by side.
			</p>
...
function MultipleSessionsSection() {
	return (
		<div className="space-y-2">
			...
			<p className="text-sm text-text-muted">
				Demonstrates the scrollable layout when there are multiple running
				sessions.
			</p>
			<MemoryRouter>
...
function MixedStatesSection() {
	return (
		<div className="space-y-2">
			...
			<p className="text-sm text-text-muted">
				Sessions with different output states: with output, no output, output
				unavailable.
			</p>
			<MemoryRouter>
				<ActiveSessionsLayout>
...
const Showcase: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getAllByText("Active Sessions").length,
...
const Loading: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByTestId("active-sessions-skeleton"),
		).toBeInTheDocument();
		const skeletonCards = canvas.getAllByTestId("session-card-skeleton");
		await expect(skeletonCards.length).toBe(SKELETON_CARD_COUNT);
	},
...
const SingleSession: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		await expect(link).toHaveAttribute(
			"href",
			"/epic/my-epic/story/my-story?tab=sessions",
		);
	},
};
