  ...
  return (
    <section>
      ...
      <div className="space-y-4">
        <div>
          <p className="text-sm text-text-muted mb-2">Running Session (expanded)</p>
          <SessionDetailCard session={runningSession} defaultExpanded={true} />
        </div>
        <div>
          <p className="text-sm text-text-muted mb-2">Completed Session (collapsed)</p>
        ...
        <div>
          ...
          <SessionDetailCard session={unavailableOutputSession} defaultExpanded={false} />
        </div>
      </div>
    </section>
  );
...
export const Showcase: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByTestId('sessions-panel-skeleton')).toBeInTheDocument();
    const skeletonCards = canvas.getAllByTestId('session-card-skeleton');
    await expect(skeletonCards.length).toBeGreaterThanOrEqual(MIN_SKELETON_CARDS);
    ...
    await expect(sessionCards.length).toBeGreaterThanOrEqual(EXPECTED_SESSION_CARDS);
...
export const Playground: Story = {
  ...
  render: (args) => (
    <div className="space-y-4">
      ...
      <SessionDetailCard session={args.session} defaultExpanded={args.defaultExpanded} />
    </div>
  ),
  argTypes: {
    session: {
