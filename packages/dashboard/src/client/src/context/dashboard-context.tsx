...
function useDataActions(actorRef: DashboardActorRef) {
	...
	const setSessions = useCallback(
		(sessions: SessionInfo[]) =>
			actorRef.send({ type: "SESSIONS_LOADED", sessions }),
		[actorRef],
	);
...
function useDashboardState() {
	...
	const isLoading = useDashboardSelector((snapshot) =>
		snapshot.matches({ active: "loading" }),
	);
	const isConnected = useDashboardSelector((snapshot) =>
		snapshot.matches({ active: "connected" }),
	);
	const isReconnecting = useDashboardSelector((snapshot) =>
