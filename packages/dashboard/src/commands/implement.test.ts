...
describe("implement command", () => {
	...
	function createSagaProject(
		dir: string,
		options: { epicSlug: string; storySlug: string },
	) {
...
`,
			...
			const result = (() => {
				try {
					const stdout = execSync(
						`node ${cliPath} implement test-story --path ${testDir}`,
						{
							cwd: subDir,
							encoding: "utf-8",
							stdio: ["pipe", "pipe", "pipe"],
	...
	describe("detached mode", () => {
		it("should run detached by default (requires tmux)", () => {
			...
			writeFileSync(
				join(skillDir, "worker-prompt.md"),
				"# Worker Prompt\nTest prompt content",
			);
	...
	describe("dry-run mode", () => {
		...
		it("should pass all checks when environment is complete", () => {
			...
			const result = runCli(
				["implement", "test-story", "--dry-run", "--path", testDir],
				{
					env: { SAGA_PLUGIN_ROOT: pluginDir },
				},
			);
		...
		it("should fail when worker-prompt.md is missing", () => {
			...
			mkdirSync(join(pluginDir, "skills", "execute-story"), {
				recursive: true,
			});
		...
		it("should report story found successfully", () => {
			...
			const result = runCli(
				["implement", "my-story", "--dry-run", "--path", testDir],
				{
					env: { SAGA_PLUGIN_ROOT: pluginDir },
				},
			);
