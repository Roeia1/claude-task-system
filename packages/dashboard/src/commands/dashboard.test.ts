...
describe("saga dashboard command", () => {
	...
	function startCliAsync(
		...
		return new Promise((resolve, reject) => {
			...
			proc.stdout?.on("data", (data) => {
				...
				const serverStarted = stdout.includes(
					"SAGA Dashboard server running on",
				);
				const projectShown =
					!options.waitForProject || stdout.includes("Project:");
	...
	it("starts server and shows project path", async () => {
		...
		try {
			expect(stdout).toContain(
				`SAGA Dashboard server running on http://localhost:${port}`,
			);
	...
	it("resolves project path with global --path option", async () => {
		...
		const { proc, stdout } = await startCliAsync(
			`--path ${projectDir} dashboard --port ${port}`,
			{
				cwd: tempDir,
				waitForProject: true,
			},
