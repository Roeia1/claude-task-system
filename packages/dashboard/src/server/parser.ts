...
function parseTasks(
	tasks: unknown,
): Array<{ id: string; title: string; status: ApiTaskStatus }> {
...
async function toStoryDetail(
	...
	return {
		...
		paths: {
			...
			...(story.journalPath
				? { journalMd: relative(sagaRoot, story.journalPath) }
				: {}),
			...(story.worktreePath
				? { worktree: relative(sagaRoot, story.worktreePath) }
				: {}),
		},
	};
}
