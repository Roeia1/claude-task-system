...
async function createTempSagaDir(): Promise<string> {
	...
	await mkdir(
		join(tempDir, ".saga", "epics", "test-epic", "stories", "test-story"),
		{
			recursive: true,
		},
	);
...
describe("integration", () => {
	...
	beforeEach(async () => {
		...
		await new Promise((resolve) =>
			setTimeout(resolve, WATCHER_SETTLE_DELAY_MS),
		);
	});
...
`,
		...
		it("should deliver epic update to all clients when structure changes", async () => {
			...
			await mkdir(
				join(tempDir, ".saga", "epics", "test-epic", "stories", "new-story"),
				{
					recursive: true,
				},
			);
...
`,
		...
		it("should update story via API after file change is detected", async () => {
			...
			const initialRes = await request(server.app).get(
...
`,
			...
			const updatedRes = await request(server.app).get(
				"/api/stories/test-epic/test-story",
			);
...
`,
			...
			const messages = await Promise.all(
				clients.map((ws) =>
					waitForEvent(ws, "story:updated", EVENT_TIMEOUT_MS),
				),
			);
...
`,
			...
			// Get API response
			await new Promise((resolve) => setTimeout(resolve, SHORT_DELAY_MS));
			const apiRes = await request(server.app).get(
				"/api/stories/test-epic/test-story",
			);
