...
describe("session routes", () => {
	...
	function createMockSession(
		overrides: Partial<DetailedSessionInfo> = {},
	): DetailedSessionInfo {
		return {
	...
	describe("GET /api/sessions", () => {
		...
		it("should filter by epicSlug", async () => {
			...
			expect(res.body.length).toBe(EXPECTED_COUNT_TWO);
			expect(
				res.body.every((s: DetailedSessionInfo) => s.epicSlug === "epic1"),
			).toBe(true);
		});
		...
		it("should filter by storySlug (requires epicSlug)", async () => {
			...
			const res = await request(app).get(
				sessionsPath({ epicSlug: "epic1", storySlug: "story1" }),
			);
		...
		it("should filter by status=running", async () => {
			...
			expect(res.body.length).toBe(EXPECTED_COUNT_TWO);
			expect(
				res.body.every((s: DetailedSessionInfo) => s.status === "running"),
			).toBe(true);
		});
	...
	describe("GET /api/sessions/:sessionName", () => {
		it("should return session by exact name", async () => {
			...
			const res = await request(app).get(
				"/api/sessions/saga__test-epic__test-story__12345",
			);
		...
		it("should require exact name match", async () => {
			...
			const res = await request(app).get(
				"/api/sessions/saga__test-epic__test-story",
			);
