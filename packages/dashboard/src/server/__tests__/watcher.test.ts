...
describe("watcher", () => {
	...
	beforeEach(async () => {
		...
		await mkdir(
			join(sagaRoot, ".saga", "epics", "test-epic", "stories", "test-story"),
			{
				recursive: true,
			},
		);
	...
	describe("file change detection", () => {
		it("should emit story:changed event when story.md is modified", async () => {
			...
			await new Promise((resolve) =>
				setTimeout(resolve, WATCHER_READY_DELAY_MS),
			);
...
`,
		...
		it("should emit epic:added event when new epic directory is created", async () => {
			...
			expect(
				events.some(
					(e) => e.type === "epic:added" && e.epicSlug === "new-epic",
				),
			).toBe(true);
		...
		it("should emit story:added event when new story is created", async () => {
			...
			// Create new story
			await mkdir(
				join(sagaRoot, ".saga", "epics", "test-epic", "stories", "new-story"),
				{
					recursive: true,
				},
			);
...
`,
			...
			expect(
				events.some(
					(e) => e.type === "story:added" && e.storySlug === "new-story",
				),
			).toBe(true);
	...
	describe("archive watching", () => {
		it("should emit story:changed event for archived story changes", async () => {
			...
			await mkdir(
				join(sagaRoot, ".saga", "archive", "test-epic", "archived-story"),
				{
					recursive: true,
				},
			);
