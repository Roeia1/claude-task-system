...
`;
			...
			const result = assertDefined(
				await parseStory(join(storyPath, "story.md"), "test-epic"),
			);
...
`,
			...
			const activeStory = result[0].stories.find(
				(s) => s.slug === "active-story",
			);
			const archivedStory = result[0].stories.find(
				(s) => s.slug === "archived-story",
			);
