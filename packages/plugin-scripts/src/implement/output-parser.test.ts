...
describe('output-parser', () => {
  describe('WORKER_OUTPUT_SCHEMA', () => {
    ...
    it('defines blocker as optional string or null', () => {
      expect(WORKER_OUTPUT_SCHEMA.properties.blocker.type).toEqual(['string', 'null']);
    });
  });
  ...
  describe('formatToolUsage', () => {
    ...
    it('formats Read tool with offset and limit', () => {
      ...
      expect(result).toBe('[Tool Used: Read] /path/to/file.ts (offset=10, limit=50)');
    });
    ...
    it('formats Read tool with only offset', () => {
      ...
      expect(result).toBe('[Tool Used: Read] /path/to/file.ts (offset=10)');
    });

    it('formats Write tool with file path', () => {
      const result = formatToolUsage('Write', { file_path: '/path/to/file.ts' });
    ...
    it('formats Glob tool with pattern and path', () => {
      const result = formatToolUsage('Glob', { pattern: '**/*.ts', path: '/src' });
    ...
    it('formats StructuredOutput tool with status and summary', () => {
      ...
      expect(result).toBe('[Tool Used: StructuredOutput] FINISH - Task completed');
    });

    it('formats unknown tools with all fields', () => {
      const result = formatToolUsage('CustomTool', { arg1: 'value1', arg2: 42 });
    ...
    it('handles null input gracefully', () => {
      const result = formatToolUsage('Read', null as unknown as Record<string, unknown>);
      expect(result).toBe('[Tool Used: Read] unknown');
    });

    it('handles undefined input gracefully', () => {
      const result = formatToolUsage('Read', undefined as unknown as Record<string, unknown>);
      expect(result).toBe('[Tool Used: Read] unknown');
    });
    ...
    it('handles multiline strings by replacing newlines', () => {
      const result = formatToolUsage('CustomTool', { text: 'line1\nline2\nline3' });
  ...
  describe('validateAndExtractOutput', () => {
    it('extracts valid FINISH output', () => {
      const output = { status: 'FINISH', summary: 'Task completed', blocker: null };
    ...
    it('throws for invalid status', () => {
      ...
      expect(() => validateAndExtractOutput(output)).toThrow('Invalid status: INVALID');
    });
  ...
  describe('processResultLine', () => {
    ...
    it('throws for error results', () => {
      ...
      expect(() => processResultLine(data, [])).toThrow('Worker failed: Something went wrong');
    });
  ...
  describe('parseStreamingResult', () => {
    ...
    it('throws when no result line found', () => {
      ...
      expect(() => parseStreamingResult(buffer)).toThrow('No result found in worker output');
    });

    it('handles empty buffer', () => {
      expect(() => parseStreamingResult('')).toThrow('No result found in worker output');
    });
    ...
    it('propagates worker error messages', () => {
      ...
      expect(() => parseStreamingResult(buffer)).toThrow('Worker failed: API rate limit exceeded');
    });
  });
});
