...
describe("EpicPaths", () => {
	it("creates correct epic paths", () => {
		...
		expect(paths.storiesDir).toBe(
			"/path/to/project/.saga/epics/my-epic/stories",
		);
	});

	it("handles epic slug with special characters", () => {
		const paths: EpicPaths = createEpicPaths(
			"/path/to/project",
			"cli-dashboard-refactor",
		);

		expect(paths.epicSlug).toBe("cli-dashboard-refactor");
		expect(paths.epicDir).toBe(
			"/path/to/project/.saga/epics/cli-dashboard-refactor",
		);
	});
});
...
describe("StoryPaths", () => {
	it("creates correct story paths", () => {
		const paths: StoryPaths = createStoryPaths(
			"/path/to/project",
			"my-epic",
			"my-story",
		);
		...
		expect(paths.storyDir).toBe(
			"/path/to/project/.saga/epics/my-epic/stories/my-story",
		);
		expect(paths.storyMd).toBe(
			"/path/to/project/.saga/epics/my-epic/stories/my-story/story.md",
		);
...
describe("WorktreePaths", () => {
	it("creates correct worktree paths", () => {
		const paths: WorktreePaths = createWorktreePaths(
			"/path/to/project",
			"my-epic",
			"my-story",
		);
		...
		expect(paths.worktreeDir).toBe(
			"/path/to/project/.saga/worktrees/my-epic/my-story",
		);
	});
...
describe("ArchivePaths", () => {
	it("creates correct archive paths for epic", () => {
		const paths: ArchivePaths = createArchivePaths(
			"/path/to/project",
			"my-epic",
		);
	...
	it("creates correct archive paths for story", () => {
		...
		expect(paths.epicSlug).toBe("my-epic");
		expect(paths.storySlug).toBe("my-story");
		expect(paths.archiveStoryDir).toBe(
			"/path/to/project/.saga/archive/my-epic/my-story",
		);
		expect(paths.archiveStoryMd).toBe(
			"/path/to/project/.saga/archive/my-epic/my-story/story.md",
		);
	});
});
