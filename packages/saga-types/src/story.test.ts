...
describe('StoryStatusSchema', () => {
  it('accepts valid status values', () => {
    const validStatuses: StoryStatus[] = ['ready', 'in_progress', 'blocked', 'completed'];
...
describe('TaskSchema', () => {
  ...
  it('requires all fields', () => {
    ...
    expect(() => TaskSchema.parse({ title: 'Test', status: 'pending' })).toThrow(); // missing id
  });
});
...
describe('StoryFrontmatterSchema', () => {
  ...
  it('parses valid frontmatter', () => {
    expect(StoryFrontmatterSchema.parse(validFrontmatter)).toEqual(validFrontmatter);
  });
