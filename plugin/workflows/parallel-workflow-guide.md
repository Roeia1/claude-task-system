...
2. Navigate to the worktree: `cd task-system/worktrees/task-XXX-feature/`
...
/task-system:parallel-start-task 010
```
...
/task-system:parallel-start-task 011
```

### Scenario 3: Multiple Small Tasks
```bash
# Work on several small refactoring tasks
/task-system:parallel-start-task  # Choose task 1
/task-system:parallel-start-task  # Choose task 2
/task-system:parallel-start-task  # Choose task 3
```
...
/task-system:parallel-start-task 016
cd task-system/worktrees/task-016-bugfix
# Fix and complete quickly...
/task-system:parallel-finalize-task

# Later, from main repository:
cd /path/to/project
/task-system:parallel-cleanup-worktree 016
```
...
- Check `task-system/worktrees/` for the active worktree
...
| Start Command | `/task-system:start-task` | `/task-system:parallel-start-task` |
| Complete Command | `/task-system:complete-task` | `/task-system:parallel-finalize-task` + `/task-system:parallel-cleanup-worktree` |
...
- **NEW**: Task completion is now split into two commands:
  ...
  - `/task-system:parallel-cleanup-worktree` - Run from main repository to remove worktree
