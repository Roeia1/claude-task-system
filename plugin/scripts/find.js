...
function getEpicSlugs(projectPath) {
  const epicsDir = join2(projectPath, ".saga", "epics");
  return readdirSync(epicsDir, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name);
}
...
function resolveProjectPath(explicitPath) {
  ...
  const sagaDir = join3(targetPath, ".saga");
  if (!existsSync2(sagaDir)) {
    throw new Error(
