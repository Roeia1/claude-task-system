...
function checkStoryMdExists(storyInfo) {
  ...
  const exists = existsSync4(worktreePaths.storyMdInWorktree);
  return {
...
function runDryRun(storyInfo) {
  ...
  const worktreePath = projectDir ? createWorktreePaths(projectDir, storyInfo.epicSlug, storyInfo.storySlug).worktreeDir : "";
  return {
    success: allPassed,
    checks,
    story: {
...
function handleDryRun(storyInfo) {
...
async function handleInternalSession(storyInfo, options) {
...
async function implementCommand(storySlug, options) {
  ...
  const storyInfo = await findStory2(storySlug);
  if (!storyInfo) {
  ...
  const isInternalSession = process3.env.SAGA_INTERNAL_SESSION === "1";
