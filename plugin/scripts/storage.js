...
function writeEpic(projectRoot, epic) {
...
function readEpic(projectRoot, epicId) {
...
function readTask(projectRoot, storyId, taskId) {
  const { storyDir } = createStoryPaths(projectRoot, storyId);
...
function listTasks(projectRoot, storyId) {
  const { storyDir } = createStoryPaths(projectRoot, storyId);
  if (!existsSync(storyDir)) {
    throw new Error(`Story directory not found: ${storyDir}`);
  }
  const files = readdirSync(storyDir);
  return files.filter((f) => f.endsWith(".json") && f !== "story.json").map((f) => {
...
function listEpics(projectRoot) {
  const { epics } = createSagaPaths(projectRoot);
  if (!existsSync(epics)) {
...
function listEpicStories(projectRoot, epicId) {
  return listStories(projectRoot).filter((story) => story.epic === epicId);
}
function listStandaloneStories(projectRoot) {
  return listStories(projectRoot).filter((story) => story.epic === void 0);
}
...
function ensureUniqueStoryId(projectRoot, id) {
