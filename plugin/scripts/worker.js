...
function createDraftPr(storyId, worktreePath) {
  const branch = `story/${storyId}`;
  const existing = findExistingPr(branch, worktreePath);
  if (existing) {
    process2.stdout.write(`[worker] PR already exists: ${existing.url}
`);
...
Created by SAGA worker.`
    ...
    process2.stdout.write(`[worker] Created draft PR: ${prUrl}
`);
...
var EXIT_SUCCESS = 0;
var EXIT_TIMEOUT = 2;
function markPrReady(storyId, worktreePath, allCompleted) {
  if (!allCompleted) {
    return;
  }
  const branch = `story/${storyId}`;
  try {
    execFileSync2("gh", ["pr", "ready", branch], {
      cwd: worktreePath,
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"]
    });
    process4.stdout.write(`[worker] PR marked as ready for review: ${branch}
`);
  } catch {
    process4.stderr.write(`[worker] Warning: Could not mark PR as ready for ${branch}
`);
  }
}
function buildStatusSummary(allCompleted, cycles, elapsedMinutes) {
  return {
...
function spawnHeadlessRun(prompt, model, taskListId, storyId, worktreePath) {
  return new Promise((resolve) => {
    ...
    child.stdout?.on("data", (chunk) => {
      process5.stdout.write(chunk);
    });
    child.stderr?.on("data", (chunk) => {
      process5.stderr.write(chunk);
    });
    child.on("error", (err) => {
      process5.stderr.write(`[worker] Headless run error: ${err.message}
`);
      resolve({ exitCode: 1 });
    });
    child.on("close", (code) => {
...
function executeCycle(config, state) {
...
`);
  ...
  mkdirSync3(dirname2(worktreeDir), { recursive: true });
  if (branchExists(branch, projectDir)) {
    process6.stdout.write(`[worker] Branch ${branch} exists, re-creating worktree
`);
  ...
  process6.stdout.write(`[worker] Created worktree: ${worktreeDir} (branch: ${branch})
`);
...
`.trim();
  process7.stdout.write(`${usage}
`);
}
function printError(message) {
  process7.stderr.write(`Error: ${message}
`);
}
...
async function main() {
  const parsed = parseArgs(process7.argv.slice(2));
  if (!parsed) {
    process7.exit(1);
  }
  const { storyId, options } = parsed;
  process7.stdout.write(`[worker] Starting pipeline for story: ${storyId}
`);
...
`
  ...
  process7.exit(summary.exitCode);
}
main().catch((error) => {
  process7.stderr.write(`Error: ${error instanceof Error ? error.message : String(error)}
`);
  process7.exit(1);
});
export {
