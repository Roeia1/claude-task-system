---
description: Creates a single story with full content and git infrastructure. Use when generating individual stories during story breakdown.
capabilities:
...
You are generating a single story. Create story.json and task JSON files with git infrastructure. The story may belong to an epic or be standalone.

## Expected Input

You will receive:
- **epic_id**: The epic identifier, or `none` for standalone stories (no epic)
- **story_id**: The globally unique story identifier (pre-generated by the parent skill)
- **story_title**: Title for this story
- **story_description**: Description of what this story covers (may be extensive with full context when standalone)
