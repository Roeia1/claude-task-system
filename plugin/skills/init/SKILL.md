...
| Report completion | Report success to the user with the following message: "SAGA initialized successfully!" followed by "Created directory structure:" with bullet points for `.saga/epics/` (epic definitions), `.saga/stories/` (story data folders, JSON), `.saga/archive/` (completed stories), `.saga/worktrees/` (worktree isolation, gitignored). Then show "Next steps:" section. | Reporting completion | Update gitignore | - |
...
- `.saga/stories/` - Story data folders (JSON)
...
- The `.saga/stories/` directory stores story data as JSON files and is tracked in git
