...
When activated, help the user create a feature definition document that describes WHAT needs to be built (not HOW). This uses AI to help structure requirements, user stories, and acceptance criteria while identifying ambiguities that need clarification.
...
1. **Get feature description from user input**
   - User provides description like "define feature user authentication system"
   - The description should be a brief summary of what needs to be built

2. **Determine next feature ID**
   - List existing features in `task-system/features/` directory
...
4. **Create directory structure**
   - Create `task-system/features/NNN-slug/`
...
9. **Create feature.md file**
   - Write final content to `task-system/features/NNN-slug/feature.md`

10. **Report completion**
    ```
    Feature defined: task-system/features/001-user-authentication/
    Feature file: task-system/features/001-user-authentication/feature.md

    Next steps:
    - Review and refine feature.md if needed
    - Say "plan feature" to create technical plan
    ```

## Template Structure

The generated feature.md follows this structure:
...
Please say: "define feature [description]"

Example: "define feature user authentication with OAuth"
```
...
```
Warning: Similar feature exists: task-system/features/002-user-auth/
...
**User says**: "define feature export data to CSV"
...
**User says**: "define feature multi-tenant user management with role-based permissions"
...
After feature definition is complete, suggest running the **feature-planning** skill (say "plan feature") to create the technical implementation plan.
