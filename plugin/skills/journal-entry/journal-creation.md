...
Read the journal template from plugin's `templates/execution/journal-template.md`
...
| `{TASK_TITLE}` | Task title | Read from `task-system/tasks/{task_id}/task.md` (first heading) |
...
If plugin's `templates/execution/journal-template.md` doesn't exist:
