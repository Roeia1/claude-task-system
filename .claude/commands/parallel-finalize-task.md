...
After this command, run `/project:parallel-cleanup-worktree` from the main repository to remove the worktree.
...
This command is for parallel (worktree) tasks only. For regular tasks, use `/project:complete-task`.
...
- Use provided task ID (e.g., `/project:parallel-finalize-task 013`)
- Verify task exists and is IN_PROGRESS in TASK-LIST.md
- Check task HAS `[worktree: path]` marker
- If not worktree task, error: "Task XXX is not in a worktree. Use /project:complete-task instead."
...
3. Display numbered menu of worktree tasks:
   ...
   1. [013] Enable Parallel Task Execution (IN_PROGRESS - Phase 7) [worktree: worktrees/task-013-feature]
   2. [015] Add Search Feature (IN_PROGRESS - Phase 6) [worktree: worktrees/task-015-feature]
...
1. Remove worktree-specific CLAUDE.md additions:
   ...
   - Remove the worktree-specific section (everything before "# Claude Code Instructions")
   ...
   if grep -q "# Task .* Worktree - ISOLATED ENVIRONMENT" CLAUDE.md; then
       # Extract content starting from "# Claude Code Instructions"
...
Use /project:complete-task instead.
```
...
Use /project:complete-task for regular tasks.
```

### Worktree Access Issues
```
Error: Cannot access worktree at worktrees/task-XXX-type
...
✅ Switched to worktree: worktrees/task-XXX-type
...
2. Run: /project:parallel-cleanup-worktree XXX

The worktree will be removed and the task will be fully complete.

Current worktree: worktrees/task-XXX-type
```
...
- For regular tasks, use `/project:complete-task`