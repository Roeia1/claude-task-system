...
|---------|-------------|-------------|------------|--------|
| Gather changes | First run `git status --porcelain -- packages/cli/` to check for uncommitted changes in the CLI package. If there are uncommitted changes, ask the user to commit them first so they appear in the changelog - stop and wait for user to commit. Once clean, run: `git log --oneline --all -- packages/cli/ \| head -20` and `git tag -l "cli-v*" \| tail -5` and `grep '"version"' packages/cli/package.json`. Review commits to `packages/cli/` since the last CLI tag. Note the current version. Categorize changes as: Added (new features/commands), Changed (modified behavior), Fixed (bug fixes), Removed (deprecated/removed). This categorization will be used for the changelog entry. | Gathering changes | - | Determine version |
