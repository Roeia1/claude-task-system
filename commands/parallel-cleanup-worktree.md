...
- Task must be finalized first using `/task-system:parallel-finalize-task`
...
```
/task-system:parallel-cleanup-worktree 013
```
...
```
/task-system:parallel-cleanup-worktree --all
```
...
2. Verify by checking:
   ...
   - Current directory path doesn't contain `/task-system/worktrees/`
...
- Use provided task ID (e.g., `/task-system:parallel-cleanup-worktree 013`)
...
4. Display menu of worktrees that can be cleaned up:
   ...
   1. [013] Enable Parallel Task Execution - task-system/worktrees/task-013-feature
   2. [015] Add Search Feature - task-system/worktrees/task-015-feature
...
1. **Verify worktree exists**:
   ...
   if git worktree list | grep -q "task-system/worktrees/task-XXX-type"; then
       echo "Found worktree: task-system/worktrees/task-XXX-type"
   else
...
You are currently in: task-system/worktrees/task-XXX-type
Please run this command from the main repository.

cd [main-repo-path]
/task-system:parallel-cleanup-worktree XXX
```
...
Run /task-system:parallel-finalize-task XXX first.
```
...
Current worktrees:
- main (master)
- task-system/worktrees/task-014-refactor (task-014-refactor)
```

### Worktree Removal Failed
```
Error: Failed to remove worktree task-system/worktrees/task-XXX-type
...
- Complementary to `/task-system:parallel-finalize-task`
